...
flowchart TD
    ...
    B -->|Yes| C[❌ NEVER force push]
    B -->|No| D{Is this a shared branch?}
    D -->|Yes| E{Do collaborators know?}
    E -->|No| F[❌ Don't force push]
    E -->|Yes| G{Have they backed up?}
    G -->|No| F
    G -->|Yes| H[⚠️ Proceed with caution]
    D -->|No| I{Is this YOUR branch?}
    I -->|Yes| J[✅ Safe to force push]
    I -->|No| K[❌ Don't force push]
```
...
flowchart TD
    ...
    D -->|Yes| E[✅ Use --force-with-lease]
    D -->|No| F{Everyone notified & backed up?}
    F -->|Yes| G[⚠️ Use --force-with-lease, be careful]
    F -->|No| H[❌ Don't do it]
```
...
➡️ [Making Your First Contribution →](../first-contribution/index)
