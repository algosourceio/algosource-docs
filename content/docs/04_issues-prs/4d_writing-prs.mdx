...
import { GitPullRequest, CheckCircle, XCircle, AlertTriangle, Code, FileText } from 'lucide-react'

<Callout type="warning" title="First Impressions Matter">
...
<Callout type="info" title="Break Large Changes into Smaller PRs">
...
<Warning title="Never Submit with Broken CI">
...
<CardGrid>
<Card title="Before State" icon={<XCircle className="h-5 w-5" />}>
Show what it looked like before your changes.
</Card>

<Card title="After State" icon={<CheckCircle className="h-5 w-5" />}>
Show your implementation.
</Card>

<Card title="Different States" icon={<Code className="h-5 w-5" />}>
Loading, error, empty, populated states.
</Card>

<Card title="Responsive Views" icon={<FileText className="h-5 w-5" />}>
...
<Card title="Learn PR Templates" icon={<FileText className="h-5 w-5" />}>
